    ...
    return (
        <div className="max-w-md mx-auto p-4 bg-white shadow-lg rounded-lg mt-10">
            ...
            <div className="flex gap-2 mb-4">
                ...
                />
                <button onClick={handleAddTodo} className="bg-blue-500 text-white px-4 py-2 rounded-lg">Add</button>
            </div>
            <ul>
                {todos.map(todo => (
                    <li key={todo.id} className="flex justify-between items-center bg-gray-100 p-2 my-2 rounded-lg">
                        ...
                        <button onClick={() => dispatch(deleteTodo(todo.id))} className="bg-red-500 text-white px-2 py-1 rounded-lg">Delete</button>
                    </li>
                ))}
            </ul>
        </div>
